...
namespace glhelper
{
  ...
  GLuint create_feedback_from_file(const std::string& vs_file){

      const std::string vs_content = read_file(vs_file);
      ...
      const GLchar* feedbackVaryings[] = { "outputPosition"};
      glTransformFeedbackVaryings(program_id, 1, feedbackVaryings, GL_SEPARATE_ATTRIBS);
    ...
    GLuint create_program_compute_shader(const std::string& cs_file){

      const std::string cs_content = read_file(cs_file);
